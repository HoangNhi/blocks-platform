...
type AssistedTestnetPanelProps = {
  side: "buy" | "sell"
  sizeMode: "base" | "quote"
  amount: string
  credentialRefId: string
  symbol: string
  preview?: TradeLabTestnetOrderPreviewResult | null
  previewError?: string | null
  list?: TradeLabTestnetOrderList | null
  listError?: string | null
  sourceReady: boolean
  sourceReadyLabel?: string
  previewDisabledReason?: string | null
  submitResult?: TradeLabTestnetOrderOperationResult | null
  submitError?: string | null
  cancelResult?: TradeLabTestnetOrderOperationResult | null
  cancelError?: string | null
  reconcileResult?: TradeLabTestnetOrderOperationResult | null
  ...
  selectedIntent?: TradeLabTestnetOrderIntent | null
...
export function AssistedTestnetPanel({
  side,
  sizeMode,
  amount,
  credentialRefId,
    ...
    <Card>
      <CardHeader className="space-y-2">
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-xs text-slate-500">Binance Spot Testnet order readiness.</p>
          </div>
      ...
      <CardContent className="space-y-4">
        <Alert>
          ...
          <AlertDescription>Assisted submit, cancel, and reconcile stay Binance Spot Testnet only with explicit confirmation.</AlertDescription>
        </Alert>

        <div className="grid gap-3">
          <div className="grid gap-1.5">
            <label className="text-xs font-medium text-slate-700" htmlFor="testnet-credential-ref">
              Credential reference ID
            </label>
            <Input
              id="testnet-credential-ref"
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="grid gap-1.5">
              ...
              <Select value={side} onValueChange={(value) => onSideChange?.(value as "buy" | "sell")}>
                <SelectTrigger aria-label="Testnet order side">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
            ...
            <div className="grid gap-1.5">
              ...
              <Select value={sizeMode} onValueChange={(value) => onSizeModeChange?.(value as "base" | "quote")}>
                <SelectTrigger aria-label="Testnet order size mode">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
          ...
          <div className="grid gap-1.5">
            <label className="text-xs font-medium text-slate-700" htmlFor="testnet-order-amount">
              Amount ({sizeMode === "quote" ? "USDT" : symbol.replace(/USDT$/i, "") || "base"})
            </label>
            <Input
              id="testnet-order-amount"
              inputMode="decimal"
              value={amount}
        ...
        <div className="grid gap-2">
          <ConfirmActionDialog
            ...
            title="Confirm Binance Spot Testnet submit"
            description="Binance Spot Testnet only. This submits the previewed testnet order and does not enable production trading or request raw credentials."
            confirmLabel="Submit testnet order"
          ...
          <ConfirmActionDialog
            triggerLabel="Cancel"
            title="Cancel Binance Spot Testnet order"
            description="Binance Spot Testnet only. This requests cancellation for the selected testnet order and does not touch live exchange state."
            confirmLabel="Cancel testnet order"
            disabled={!canCancel}
          ...
          <ConfirmActionDialog
            ...
            title="Reconcile Binance Spot Testnet order"
            description="Binance Spot Testnet only. This reads exchange evidence for the selected testnet order and updates local evidence."
            confirmLabel="Reconcile testnet order"
...
function OperationResult({
  ...
  label: string
  result?: TradeLabTestnetOrderOperationResult | null
  error?: string | null
}) {
