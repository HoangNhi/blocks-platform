...
type SchedulerStatusPanelProps = {
  status: TradeLabFillSchedulerStatus | null
  isLoading?: boolean
      ...
      <CardHeader className="space-y-3 border-b border-slate-200 bg-slate-50/80">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-xs text-slate-500">Read-only background fill scheduler state.</p>
          </div>
      ...
      <CardContent className="grid gap-4 p-4">
        ...
        {status ? (
          <dl className="grid gap-2 text-xs text-slate-600 sm:grid-cols-2">
            ...
            <Field label="Job ID" value={status.lastJobId ?? "N/A"} />
            <Field label="Dataset key" value={status.lastDatasetKey ?? "N/A"} />
            <Field label="Stale jobs marked" value={String(status.staleJobsMarked)} />
...
function Field({ label, value, icon = false }: { label: string; value: string; icon?: boolean }) {
  return (
    <div className="grid gap-1 rounded-md border border-slate-200 bg-slate-50 px-2 py-1">
