...
import type { TradeLabTestnetOrderDetail, TradeLabTestnetOrderJournalProjectionResult, TradeLabTestnetOrderPreviewResult } from "../types"

type AssistedTestnetOrderDetailPanelProps = {
  detail?: TradeLabTestnetOrderDetail | null
  preview?: TradeLabTestnetOrderPreviewResult | null
  errorMessage?: string | null
  isLoading?: boolean
  canProjectToJournal?: boolean
  isProjectingToJournal?: boolean
  projectionResult?: TradeLabTestnetOrderJournalProjectionResult | null
...
export function AssistedTestnetOrderDetailPanel({
  detail = null,
  preview = null,
  errorMessage = null,
  isLoading = false,
  ...
  onProjectToJournal,
}: AssistedTestnetOrderDetailPanelProps) {
      ...
      <CardHeader className="space-y-2">
        <div className="flex items-start justify-between gap-3">
          <CardTitle className="flex items-center gap-2 text-base">
            <ClipboardList className="size-4 text-blue-600" aria-hidden="true" />
            Assisted Testnet Evidence
          </CardTitle>
          <Badge variant="secondary">Preview only</Badge>
        </div>
        <p className="text-xs text-slate-500">Read-only order intent, preview snapshots, and lifecycle audit trail.</p>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        {!detail && !preview && !isLoading ? (
          <div className="rounded-md border border-dashed p-4 text-sm text-slate-500">
            Generate a preview or select a recent assisted testnet order to inspect read-only evidence.
          </div>
        ) : null}
          ...
          <section className="space-y-4">
            ...
            <div className="rounded-md border border-dashed p-3 text-sm text-slate-600">
              <div className="flex flex-wrap items-center justify-between gap-2">
                ...
                <Badge variant="secondary">Phase 19.5B</Badge>
              </div>
              <p className="mt-2">Projection opens in Phase 19.5B after terminal order evidence exists.</p>
              ...
              <Dialog>
                ...
                <DialogContent>
                  <DialogHeader>
                    <DialogTitle>Project assisted testnet evidence</DialogTitle>
                    <DialogDescription>
                      This writes execution evidence from Binance Spot Testnet order evidence into the execution journal and does not mutate exchange state.
                    </DialogDescription>
                  </DialogHeader>
                  <DialogFooter>
