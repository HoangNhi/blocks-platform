...
trial_log_path = os.path.join(task_dir, "trial-log-1h.md")
report_path = os.path.join(task_dir, "report-1h.md")
...
def main():
    ...
    strategy_name = f"WFA 1H Strategy {suffix}"
    strategy_slug = f"wfa-1h-strategy-{suffix}"
    ...
    strat_res = post_json("/api/tradelab/strategies", {
        ...
        "name": strategy_name,
        "slug": strategy_slug,
        "description": "Programmatic WFA strategy on 1h",
        "runtime_config": {},
        "risk_config": {}
    }, token)
    ...
    with open(trial_log_path, "w", encoding="utf-8") as f:
        f.write("# TradeLab 1H WFA Trial Log\n\n")
    ...
    # ----------------------------------------------------
    # PHASE 1: IN-SAMPLE SEARCH (1h)
    # Range: 2026-01-01 to 2026-04-01 (3 months)
    # ----------------------------------------------------
    print("\n=== PHASE 1: IN-SAMPLE SEARCH (BTCUSDT 1h, 2026-01-01 to 2026-04-01) ===")
    ...
    # Range: 2026-04-01 to 2026-05-07 (36 days)
    # ----------------------------------------------------
    print("\n=== PHASE 2: VALIDATION (BTCUSDT 1h, 2026-04-01 to 2026-05-07) ===")
    ...
    print(f"\n=== BEST 1H CANDIDATE FOR OOS: {best_candidate['desc']} ===")
    
    # ----------------------------------------------------
    # PHASE 3: OUT-OF-SAMPLE HOLDOUT & STRESS TESTING (1h)
    # Range: 2026-05-07 to 2026-06-01 (25 days)
    # ----------------------------------------------------
    print("\n=== PHASE 3: OUT-OF-SAMPLE & STRESS (BTCUSDT 1h, 2026-05-07 to 2026-06-01) ===")
    ...
    # Monthly calculations: IS = 3 months, Val = 36 days (1.2 months), OOS = 25 days (0.83 months)
    is_monthly = (parent_is_run['ret'] / 3.0) if parent_is_run else 0.0
    val_monthly = best_candidate["ret"] / 1.2
    oos_monthly = oos_ret / 0.83
    ...
    with open(report_path, "w", encoding="utf-8") as r:
        r.write(f"# TradeLab 1H Strategy Research WFA Report\n\n")
        ...
        r.write(f"| **In-Sample** | 2026-01-01 to 2026-04-01 | 10 bps fee, 1 bps slip | {is_ret:.2f}% | {is_mdd:.2f}% | {is_pf:.2f} | {is_trades} | {is_monthly:.2f}% |\n")
        r.write(f"| **Validation** | 2026-04-01 to 2026-05-07 | 10 bps fee, 1 bps slip | {best_candidate['ret']:.2f}% | {best_candidate['mdd']:.2f}% | {best_candidate['pf']:.2f} | {best_candidate['trades']} | {val_monthly:.2f}% |\n")
        r.write(f"| **Out-of-Sample** | 2026-05-07 to 2026-06-01 | 6 bps fee, 1 bps slip | {oos_ret:.2f}% | {oos_mdd:.2f}% | {oos_pf:.2f} | {oos_trades} | {oos_monthly:.2f}% |\n")
        r.write(f"| **OOS Stress** | 2026-05-07 to 2026-06-01 | 15 bps fee, 5 bps slip | {stress_ret:.2f}% | {stress_mdd:.2f}% | {stress_pf:.2f} | {stress_trades} | {(stress_ret/0.83):.2f}% |\n\n")
