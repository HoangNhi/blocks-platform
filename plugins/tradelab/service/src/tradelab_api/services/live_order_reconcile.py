...
class LiveOrderReconcileRequestData:
    order_id: UUID
    confirm_live_reconcile: bool
    trigger: str = "manual"
...
def reconcile_live_order(
    ...
    request: LiveOrderReconcileRequestData,
    *,
    ...
    if intent is None:
        return _blocked("live_order_reconcile_order_not_found", 404)
    if not _intent_shape_allowed(intent) or intent.status not in ALLOWED_RECONCILE_STATUSES:
        return _blocked("live_order_reconcile_state_not_allowed", 409, intent=intent)
    ...
    if real_gate is not None:
        ...
        event = order_repository.add_event(
            intent_id=intent.id,
            preview_id=None,
            event_type="live_order_reconcile_blocked",
    ...
    started = order_repository.add_event(
        intent_id=intent.id,
        preview_id=None,
        event_type="live_order_reconcile_started",
        from_status=intent.status,
        to_status=intent.status,
        reason_code="live_order_reconcile_started",
        idempotency_key=None,
        client_order_id=intent.client_order_id,
        exchange_order_id=intent.exchange_order_id,
        actor=request.actor,
        metadata={"trigger": request.trigger, "killSwitchEnabled": request.live_order_submit_kill_switch_enabled},
    )
    ...
    if mode == "real":
        if vault_provider is None:
            return _blocked("live_order_reconcile_vault_provider_not_supported", 403, intent=intent)
        read = read_live_credential_secret_for_internal_purpose(
            ...
            request=LiveCredentialReadRequestData(
                ...
                purpose=PHASE_20_LIVE_ORDER_RECONCILE_PURPOSE,
                actor=request.actor,
        ...
        if read.status != "allowed" or read.payload is None:
            return _blocked(
                "live_order_reconcile_vault_read_blocked",
        ...
        try:
            ...
            return _blocked("live_order_reconcile_base_url_not_allowed", 403, intent=intent)
        ...
        result = FakeBinanceLiveConnector(scenario="accepted").reconcile(order_request)
...
def _validate_request_shape(request: LiveOrderReconcileRequestData) -> LiveOrderReconcileResult | None:
    if not request.confirm_live_reconcile:
        return _blocked("live_order_reconcile_confirmation_required", 400)
    if request.trigger not in ALLOWED_TRIGGERS:
        return _blocked("live_order_reconcile_trigger_invalid", 400)
    return None


def _real_mode_gate_block(request: LiveOrderReconcileRequestData) -> tuple[str, int] | None:
    mode = request.connector_mode.lower().strip()
    if mode not in {"fake", "real"}:
        return "live_order_reconcile_connector_mode_invalid", 400
    if mode == "fake":
        return None
    if not request.real_network_enabled:
        return "live_order_reconcile_real_network_not_enabled", 403
    if request.environment_name not in {"local", "development", "test"}:
        return "live_order_reconcile_environment_not_allowed", 403
    if request.binance_live_base_url.rstrip("/") != "https://api.binance.com":
        return "live_order_reconcile_base_url_not_allowed", 403
    if request.vault_provider_name != "local_dev_encrypted":
        return "live_order_reconcile_vault_provider_not_supported", 403
    return None
...
def _credential_block_reason(credential: Any) -> str | None:
    if credential is None or getattr(credential, "is_deleted", False) or not getattr(credential, "is_active", True):
        return "live_order_reconcile_credential_not_ready"
    ...
    if evidence.get("canWithdraw") or evidence.get("marginOrFuturesEnabled"):
        return "live_order_reconcile_unsafe_permissions"
    return None


def _intent_shape_allowed(intent: Any) -> bool:
    return intent.environment == "binance_live" and intent.exchange == "binance" and intent.market_type == "spot"
...
def _map_reconcile_snapshot(snapshot: Any) -> tuple[str, bool, str, str]:
    if snapshot is None or snapshot.state == LiveOrderState.RECONCILIATION_REQUIRED:
        return "reconciliation_required", True, "live_order_reconcile_ambiguous", "ambiguous"
    if snapshot.state == LiveOrderState.UNKNOWN:
        return "reconciliation_required", True, "live_order_reconcile_ambiguous", "ambiguous"
    if snapshot.state == LiveOrderState.CANCELLED:
        return "cancelled", False, "live_order_reconcile_completed", "matched"
    if snapshot.state == LiveOrderState.FILLED:
        return "filled", False, "live_order_reconcile_completed", "matched"
    if snapshot.state == LiveOrderState.PARTIALLY_FILLED:
        return "partially_filled", False, "live_order_reconcile_completed", "matched"
    if snapshot.state == LiveOrderState.REJECTED:
        return "rejected", False, "live_order_reconcile_completed", "matched"
    return "submitted", False, "live_order_reconcile_completed", "matched"
