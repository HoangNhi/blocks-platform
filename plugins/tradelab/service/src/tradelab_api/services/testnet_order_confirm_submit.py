...
REAL_SAFETY_STATUS = "assisted_testnet_real_submit_testnet_only"
READY_CREDENTIAL_STATUSES = {"stored_testnet_only", "validated_testnet_read_only", "fake_testnet_ready"}


@dataclass(frozen=True)
class TestnetOrderConfirmSubmitRequestData:
    preview_id: UUID
...
@dataclass(frozen=True)
class TestnetOrderConfirmSubmitResult:
    status: str
    reason_code: str
...
def confirm_submit_testnet_order(
    ...
    *,
    ...
    if preview is None or intent is None:
        return _blocked("testnet_order_submit_preview_not_found", 404, safety_status=_safety_status_for_request(request))
    ...
    if replay_event is not None and replay_event.event_type != "testnet_order_submit_attempted":
        return _result_from_intent(
            intent,
            preview_id=preview.id,
            reason_code="testnet_order_submit_idempotency_replayed",
    ...
    if intent.status in {"unknown", "reconciliation_required"}:
        return _blocked(
            "testnet_order_requires_reconciliation_before_submit",
            409,
            intent=intent,
            preview_id=preview.id,
            safety_status=_safety_status_for_request(request),
    ...
    if preview.status != "allowed" or intent.status != "draft_previewed":
        return _blocked("testnet_order_submit_state_not_allowed", 409, intent=intent, preview_id=preview.id)
    if preview.expires_at is not None and preview.expires_at <= datetime.now(UTC):
        return _blocked("testnet_order_submit_preview_expired", 409, intent=intent, preview_id=preview.id)
    if intent.environment != "binance_testnet" or intent.exchange != "binance" or intent.market_type != "spot" or intent.order_type != "market":
        return _blocked("testnet_order_submit_state_not_allowed", 409, intent=intent, preview_id=preview.id)
    ...
    if request.submit_kill_switch_enabled:
        event = order_repository.add_event(
            intent_id=intent.id,
            preview_id=preview.id,
            event_type="testnet_order_submit_blocked",
            from_status=intent.status,
            to_status=intent.status,
            reason_code="testnet_order_submit_kill_switch_enabled",
        ...
        return _blocked(
            "testnet_order_submit_kill_switch_enabled",
            403,
            intent=intent,
            preview_id=preview.id,
    ...
    confirmation = order_repository.add_event(
        intent_id=intent.id,
        preview_id=preview.id,
        event_type="testnet_order_confirmation_recorded",
        from_status=intent.status,
        to_status="confirmed",
        reason_code="testnet_order_confirmation_recorded",
        idempotency_key=scoped_idempotency_key,
        client_order_id=intent.client_order_id,
        exchange_order_id=intent.exchange_order_id,
        actor=request.actor,
    ...
    if request.connector_mode.lower().strip() == "real":
        if vault_provider is None:
            return _blocked("testnet_order_submit_vault_provider_not_supported", 403, intent=intent, preview_id=preview.id, safety_status=safety_status)
        read = read_testnet_credential_secret_for_internal_purpose(
            ...
            request=TestnetCredentialReadRequestData(
                ...
                purpose=PHASE_19_3B_SUBMIT_PURPOSE,
                actor=request.actor,
        ...
        if read.status != "allowed" or read.payload is None:
            return _blocked(
                "testnet_order_submit_vault_read_blocked",
        ...
        except RealBinanceSpotTestnetConnectorError as exc:
        ...
        connector = FakeBinanceTestnetConnector(scenario=_scenario_from_idempotency_key(request.idempotency_key))
    ...
    if snapshot is None or snapshot.state == TestnetOrderState.UNKNOWN:
        unknown_reason = submit.reason_code if request.connector_mode.lower().strip() == "real" else "testnet_order_submit_unknown_state"
        order_repository.update_intent_status(
            intent,
            status="unknown",
        ...
        final_event_type = "testnet_order_submit_unknown_recorded"
    ...
    elif snapshot.state == TestnetOrderState.REJECTED:
        rejected_reason = submit.reason_code if request.connector_mode.lower().strip() == "real" else "testnet_order_submit_fake_rejected"
        order_repository.update_intent_status(
            intent,
            status="rejected",
        ...
        final_event_type = "testnet_order_submit_rejected"
        result_status = "rejected"
        result_reason = rejected_reason
        exchange_status = "rejected"
        exchange_order_id = snapshot.exchange_order_id
    else:
        final_status = "partially_filled" if snapshot.state == TestnetOrderState.PARTIALLY_FILLED else "submitted"
        accepted_reason = submit.reason_code if request.connector_mode.lower().strip() == "real" else "testnet_order_submit_fake_accepted"
        order_repository.update_intent_status(
            intent,
            status=final_status,
        ...
        final_event_type = "testnet_order_submit_accepted"
...
def _validate_request_shape(request: TestnetOrderConfirmSubmitRequestData) -> TestnetOrderConfirmSubmitResult | None:
    if not request.confirm_testnet_order:
        return _blocked("testnet_order_submit_confirmation_required", 400)
    if not request.idempotency_key.strip():
        return _blocked("testnet_order_submit_idempotency_required", 400)
    if len(request.idempotency_key) > 120 or is_secret_like_key(request.idempotency_key):
        return _blocked("testnet_order_submit_idempotency_invalid", 400)
    return None


def _real_mode_gate_block(request: TestnetOrderConfirmSubmitRequestData) -> tuple[str, int] | None:
    mode = request.connector_mode.lower().strip()
    if mode not in {"fake", "real"}:
        return "testnet_order_submit_connector_mode_invalid", 400
    if mode == "fake":
        return None
    if not request.real_network_enabled:
        return "testnet_order_submit_real_network_not_enabled", 403
    if request.environment_name not in {"local", "development", "test"}:
        return "testnet_order_submit_environment_not_allowed", 403
    if request.binance_testnet_base_url.rstrip("/") != "https://testnet.binance.vision":
        return "testnet_order_submit_base_url_not_allowed", 403
    if request.vault_provider_name != "local_dev_encrypted":
        return "testnet_order_submit_vault_provider_not_supported", 403
    return None


def _safety_status_for_request(request: TestnetOrderConfirmSubmitRequestData) -> str:
...
def _credential_block_reason(credential: Any) -> str | None:
    if credential is None or getattr(credential, "is_deleted", False) or not getattr(credential, "is_active", True):
        return "testnet_order_submit_credential_not_ready"
    ...
    if evidence.get("marginOrFuturesEnabled"):
        return "testnet_order_submit_unsafe_permissions"
    return None


def _scoped_idempotency_key(request: TestnetOrderConfirmSubmitRequestData) -> str:
    return f"testnet-order-confirm-submit:{request.preview_id}:{request.idempotency_key}"
...
def _blocked(
    ...
    *,
    ...
    return TestnetOrderConfirmSubmitResult(
        status="blocked",
        reason_code=reason_code,
