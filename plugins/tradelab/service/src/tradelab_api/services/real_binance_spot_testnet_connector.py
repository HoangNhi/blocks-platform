...
BINANCE_SPOT_TESTNET_ORDER_SAFETY_STATUS = "assisted_testnet_real_submit_testnet_only"
BINANCE_TESTNET_ORDER_ENDPOINT = "POST /api/v3/order"
BINANCE_TESTNET_CANCEL_ENDPOINT = "DELETE /api/v3/order"
BINANCE_TESTNET_RECONCILE_ENDPOINT = "GET /api/v3/order"
BINANCE_SPOT_TESTNET_CANCEL_SAFETY_STATUS = "assisted_testnet_cancel_testnet_only"
BINANCE_SPOT_TESTNET_RECONCILE_SAFETY_STATUS = "assisted_testnet_reconcile_testnet_only"
BINANCE_TESTNET_DEFAULT_BASE_URL = "https://testnet.binance.vision"
ALLOWED_TESTNET_HOSTS = {"testnet.binance.vision"}


class RealBinanceSpotTestnetConnectorError(ValueError):
...
def build_signed_order_query(
    *,
    ...
    if order_request.quantity is not None:
        ...
        raise RealBinanceSpotTestnetConnectorError("testnet_order_submit_signed_request_failed")
    pairs.extend(
        [
...
class RealBinanceSpotTestnetConnector:
    def __init__(
        self,
        *,
        ...
        if parsed.scheme != "https" or parsed.hostname not in ALLOWED_TESTNET_HOSTS or base_url.rstrip("/") != BINANCE_TESTNET_DEFAULT_BASE_URL:
            raise RealBinanceSpotTestnetConnectorError("testnet_order_submit_base_url_not_allowed")
    ...
    def submit_order(
        ...
        *,
        ...
        try:
            ...
            return self._unknown_result(order_request, "testnet_order_submit_signed_request_failed", None)
        ...
        try:
            ...
            return self._unknown_result(order_request, "testnet_order_submit_binance_timeout_unknown", None)
        except httpx.TransportError:
            return self._unknown_result(order_request, "testnet_order_submit_binance_ambiguous_unknown", None)
        finally:
            if close_client:
                client.close()
        ...
        if response.status_code in {400, 401, 403}:
            return self._rejected_result(order_request, "testnet_order_submit_binance_rejected", evidence)
        if response.status_code == 429:
            return self._unknown_result(order_request, "testnet_order_submit_binance_rate_limited_unknown", evidence)
        if response.status_code >= 500:
            return self._unknown_result(order_request, "testnet_order_submit_binance_unavailable_unknown", evidence)
        return self._unknown_result(order_request, "testnet_order_submit_binance_ambiguous_unknown", evidence)
    ...
    def cancel_order(
        ...
        *,
        ...
        try:
            ...
            return self._cancel_unknown_result(order_request, "testnet_order_cancel_binance_timeout_unknown", None)
        except httpx.TransportError:
            return self._cancel_unknown_result(order_request, "testnet_order_cancel_binance_ambiguous_unknown", None)
        finally:
            if close_client:
                client.close()
        ...
        if isinstance(body, dict) and body.get("code") == -2011:
            return self._cancel_reconciliation_required(
                order_request,
                "testnet_order_cancel_binance_not_found_reconciliation_required",
                evidence,
            )
        if response.status_code in {429} or response.status_code >= 500:
            return self._cancel_unknown_result(order_request, "testnet_order_cancel_binance_ambiguous_unknown", evidence)
        return self._cancel_reconciliation_required(
            order_request,
            "testnet_order_cancel_binance_not_found_reconciliation_required",
            evidence,
    ...
    def reconcile_order(
        ...
        *,
        ...
        try:
            ...
            return self._reconcile_required_result(order_request, "testnet_order_reconcile_binance_ambiguous", None)
        except httpx.TransportError:
            return self._reconcile_required_result(order_request, "testnet_order_reconcile_binance_ambiguous", None)
        finally:
            if close_client:
        ...
        if isinstance(body, dict) and body.get("code") in {-2011, -2013}:
            return self._reconcile_required_result(order_request, "testnet_order_reconcile_binance_not_found", evidence)
        return self._reconcile_required_result(order_request, "testnet_order_reconcile_binance_ambiguous", evidence)
    ...
    def _success_or_unknown(
        ...
        if not isinstance(body, dict) or body.get("orderId") is None:
            return self._unknown_result(order_request, "testnet_order_submit_binance_malformed_response_unknown", evidence)
    ...
    def _cancel_success_or_unknown(
        ...
        snapshot = self._snapshot_from_body(order_request, body, default_reason="testnet_order_cancel_binance_accepted")
        if snapshot is None:
            return self._cancel_unknown_result(order_request, "testnet_order_cancel_binance_ambiguous_unknown", evidence)
        reason = "testnet_order_cancel_binance_accepted"
        outcome = ConnectorOutcome.CANCELLED
        if snapshot.state in {TestnetOrderState.FILLED, TestnetOrderState.PARTIALLY_FILLED}:
            reason = "testnet_order_cancel_binance_filled_race"
            outcome = ConnectorOutcome.FILLED if snapshot.state == TestnetOrderState.FILLED else ConnectorOutcome.PARTIALLY_FILLED
        return ConnectorCancelResult(
            ...
            metadata=self._operation_metadata(
                evidence,
                endpoint=BINANCE_TESTNET_CANCEL_ENDPOINT,
                safety_status=BINANCE_SPOT_TESTNET_CANCEL_SAFETY_STATUS,
    ...
    def _reconcile_success_or_required(
        ...
        snapshot = self._snapshot_from_body(order_request, body, default_reason="testnet_order_reconcile_binance_matched")
        if snapshot is None:
            return self._reconcile_required_result(order_request, "testnet_order_reconcile_binance_ambiguous", evidence)
        return ConnectorReconciliationResult(
            ...
            metadata=self._operation_metadata(
                evidence,
                endpoint=BINANCE_TESTNET_RECONCILE_ENDPOINT,
                safety_status=BINANCE_SPOT_TESTNET_RECONCILE_SAFETY_STATUS,
                exchange_order_status=snapshot.metadata.get("exchangeOrderStatus"),
            ),
        )
    ...
    def _metadata(self, evidence: SubmitHttpEvidence | None, **extra: Any) -> dict[str, Any]:
        values = {
            "safetyStatus": BINANCE_SPOT_TESTNET_ORDER_SAFETY_STATUS,
            "networkCall": True,
            "endpoint": BINANCE_TESTNET_ORDER_ENDPOINT,
...
def _state_from_binance_status(status: str) -> TestnetOrderState:
    if status == "NEW":
        return TestnetOrderState.SUBMITTED
    if status == "PARTIALLY_FILLED":
        return TestnetOrderState.PARTIALLY_FILLED
    if status == "FILLED":
        return TestnetOrderState.FILLED
    if status == "CANCELED":
        return TestnetOrderState.CANCELLED
    if status in {"REJECTED", "EXPIRED", "EXPIRED_IN_MATCH"}:
        return TestnetOrderState.REJECTED
    return TestnetOrderState.RECONCILIATION_REQUIRED

def _outcome_from_state(state: TestnetOrderState) -> ConnectorOutcome:
    if state == TestnetOrderState.CANCELLED:
        return ConnectorOutcome.CANCELLED
    if state == TestnetOrderState.FILLED:
        return ConnectorOutcome.FILLED
    if state == TestnetOrderState.PARTIALLY_FILLED:
        return ConnectorOutcome.PARTIALLY_FILLED
    if state == TestnetOrderState.REJECTED:
        return ConnectorOutcome.REJECTED
    if state == TestnetOrderState.SUBMITTED:
