...
LIVE_CREDENTIAL_SAFETY_STATUS = "assisted_live_credential_vault_only"
LIVE_CREDENTIAL_ENVIRONMENT = "binance_live"
LIVE_CREDENTIAL_EXCHANGE = "binance_spot"
FAKE_VAULT_PROVIDER = "fake"
LOCAL_DEV_VAULT_PROVIDER = "local_dev_encrypted"
LIVE_VALIDATION_PURPOSE = "phase_20_live_credential_validation"
PHASE_20_LIVE_ORDER_SUBMIT_PURPOSE = "phase_20_live_order_submit"
PHASE_20_LIVE_ORDER_CANCEL_PURPOSE = "phase_20_live_order_cancel"
PHASE_20_LIVE_ORDER_RECONCILE_PURPOSE = "phase_20_live_order_reconcile"
ALLOWED_LIVE_CREDENTIAL_READ_PURPOSES = {
    LIVE_VALIDATION_PURPOSE,
    PHASE_20_LIVE_ORDER_SUBMIT_PURPOSE,
    PHASE_20_LIVE_ORDER_CANCEL_PURPOSE,
    PHASE_20_LIVE_ORDER_RECONCILE_PURPOSE,
}


class LiveCredentialValidationError(ValueError):
...
@dataclass(frozen=True)
class LiveCredentialSecretRequestData:
    api_key: str
    api_secret: str


@dataclass(frozen=True)
class LiveCredentialReadRequestData:
...
@dataclass(frozen=True)
class LiveCredentialReadResult:
    status: str
    reason_code: str
...
class CredentialVaultProvider(Protocol):
    def create_secret(
        ...
        *,
        ...
        actor: str,
        idempotency_key: str,
        secret: LiveCredentialSecretRequestData | None = None,
...
class FakeCredentialVaultProvider:
    def create_secret(
        ...
        *,
        ...
        if secret is not None:
            raise LiveCredentialValidationError(
                "live_credential_secret_not_allowed",
                "Fake live credential vault does not accept secret material.",
                status_code=400,
            )
        return VaultSecretWriteResult(
            vault_provider=FAKE_VAULT_PROVIDER,
            vault_secret_ref=f"fake://binance_live/{uuid4()}",
    ...
    def read_secret_for_connector(self, *, vault_secret_ref: str, purpose: str, actor: str) -> None:
        raise LiveCredentialValidationError(
            "live_credential_vault_read_blocked",
            "Fake live credential vault does not expose connector secrets.",
            status_code=403,
        )
...
class LocalDevEncryptedCredentialVaultProvider:
    ...
    def create_secret(
        ...
        *,
        ...
        if secret is None:
            raise LiveCredentialValidationError(
                "live_credential_secret_required",
                "Local/dev encrypted live credential requires apiKey and apiSecret.",
                status_code=400,
            )
        ...
        return VaultSecretWriteResult(
            ...
            vault_secret_ref=f"local-dev://binance_live/{uuid4()}",
...
@dataclass(frozen=True)
class LiveCredentialCreateRequestData:
    label: str
    confirm_create: bool
    idempotency_key: str
    actor: str = "local-user"
...
@dataclass(frozen=True)
class LiveCredentialValidateRequestData:
...
@dataclass(frozen=True)
class LiveCredentialRotateRequestData:
    confirm_rotate: bool
    idempotency_key: str
    actor: str = "local-user"
    secret: LiveCredentialSecretRequestData | None = None
...
@dataclass(frozen=True)
class LiveCredentialRevokeRequestData:
    confirm_revoke: bool
    idempotency_key: str
    actor: str = "local-user"
...
@dataclass(frozen=True)
class LiveCredentialMutationResult:
    status: str
    reason_code: str
    safety_status: str = LIVE_CREDENTIAL_SAFETY_STATUS
...
def build_fake_permission_evidence(*, can_withdraw: bool, margin_or_futures_enabled: bool) -> dict[str, Any]:
    return {
        ...
        "environment": LIVE_CREDENTIAL_ENVIRONMENT,
...
def evaluate_permission_evidence_status(evidence: dict[str, Any]) -> str:
    ...
    return "stored_live_only"


def _blocked_secret_result(fields: list[str]) -> LiveCredentialMutationResult:
    return LiveCredentialMutationResult(
        status="blocked",
        reason_code="live_credential_secret_not_allowed",
...
def _safety_status_for_provider(provider_name: str) -> str:
    return LIVE_CREDENTIAL_SAFETY_STATUS
...
def _status_blocks_secret_read(*, status: str, purpose: str) -> bool:
    if status in {"revoked", "vault_unavailable"}:
        return True
    if status == "unsafe_permissions" and purpose != LIVE_VALIDATION_PURPOSE:
        return True
    return False
...
def create_live_credential(
    ...
    *,
    ...
    if not request.confirm_create:
        return LiveCredentialMutationResult(status="blocked", reason_code="live_credential_create_confirmation_required", semantic_status_code=400)
    try:
    ...
    except LiveCredentialValidationError as exc:
        return LiveCredentialMutationResult(status="blocked", reason_code=exc.reason_code, should_commit=False, semantic_status_code=exc.status_code)

    provider_metadata = _provider_metadata_without_secret(write.metadata)
    credential = repository.create_credential_ref(
        exchange=LIVE_CREDENTIAL_EXCHANGE,
        environment=LIVE_CREDENTIAL_ENVIRONMENT,
        label=request.label,
    ...
    reason_code = "live_credential_fake_created"
    ...
    if write.vault_provider == LOCAL_DEV_VAULT_PROVIDER:
        ...
        if encrypted_payload is None:
            return LiveCredentialMutationResult(status="blocked", reason_code="live_credential_vault_write_failed", should_commit=False, semantic_status_code=500)
        ...
        reason_code = "live_credential_secret_encrypted"
        audit_metadata = {"localDevEncrypted": True, "providerMetadata": provider_metadata}
    audit = repository.add_audit_event(
        credential_ref_id=credential.id,
        action="live_credential_created",
        actor=request.actor,
        environment=LIVE_CREDENTIAL_ENVIRONMENT,
        reason_code=reason_code,
        idempotency_key_hash=hash_idempotency_key(f"live-credential:create:{request.idempotency_key}"),
...
def _validation_gate_block(reason_code: str, *, semantic_status_code: int = 400) -> LiveCredentialMutationResult:
    return LiveCredentialMutationResult(
        status="blocked",
        reason_code=reason_code,
        should_commit=False,
...
def _real_validation_action(status: str) -> str:
    if status == "passed":
        return "live_credential_validation_completed"
    if status == "blocked":
        return "live_credential_validation_blocked"
    return "live_credential_validation_failed"


def validate_live_credential(
    repository: Any,
    credential_ref_id: UUID,
    *,
    request: LiveCredentialValidateRequestData,
    provider: CredentialVaultProvider | None = None,
    validation_client: BinanceLiveAccountValidationClient | None = None,
    settings: Any | None = None,
    request_time_ms: int | None = None,
) -> LiveCredentialMutationResult:
    credential = repository.get_credential_ref(credential_ref_id)
    if credential is None:
        return LiveCredentialMutationResult(status="not_found", reason_code="live_credential_not_found", semantic_status_code=404)
    if not request.confirm_validate:
        return LiveCredentialMutationResult(status="blocked", reason_code="live_credential_validate_confirmation_required", semantic_status_code=400)
    if provider is not None and validation_client is not None and settings is not None and request_time_ms is not None:
        if getattr(settings, "tradelab_live_credential_validation_enabled", False) is not True:
            return _validation_gate_block("live_credential_validation_not_enabled")
        if getattr(settings, "tradelab_environment", "") not in {"local", "development", "test"}:
            return _validation_gate_block("live_credential_validation_environment_not_allowed")
        if getattr(settings, "tradelab_live_credential_vault_provider", "") != LOCAL_DEV_VAULT_PROVIDER:
            return _validation_gate_block("live_credential_validation_provider_not_supported")
        if getattr(settings, "tradelab_binance_live_base_url", "") != "https://api.binance.com":
            return _validation_gate_block("live_credential_validation_base_url_not_allowed")
        if credential.vault_provider != LOCAL_DEV_VAULT_PROVIDER or not isinstance(provider, LocalDevEncryptedCredentialVaultProvider):
            return _validation_gate_block("live_credential_validation_provider_not_supported")
        if credential.environment != LIVE_CREDENTIAL_ENVIRONMENT:
            return _validation_gate_block("live_credential_validation_environment_mismatch")
        if not getattr(credential, "is_active", False):
            return _validation_gate_block("live_credential_inactive")
        if getattr(credential, "is_deleted", False):
            return _validation_gate_block("live_credential_deleted")
        if credential.status == "revoked":
            return _validation_gate_block("live_credential_revoked")
        ...
        read = read_live_credential_secret_for_internal_purpose(
            ...
            request=LiveCredentialReadRequestData(
                credential_ref_id=credential.id,
                purpose=LIVE_VALIDATION_PURPOSE,
                actor=request.actor,
            ),
        )
        ...
        started = repository.add_audit_event(
            credential_ref_id=credential.id,
            action="live_credential_validation_started",
            actor=request.actor,
            environment=LIVE_CREDENTIAL_ENVIRONMENT,
            reason_code="live_credential_validation_started",
            idempotency_key_hash=hash_idempotency_key(f"live-credential:validate:{request.idempotency_key}:started"),
            metadata={"purpose": LIVE_VALIDATION_PURPOSE},
        )
        probe = validation_client.validate_account(
            api_key=read.payload["apiKey"],
            api_secret=read.payload["apiSecret"],
            recv_window_ms=getattr(settings, "tradelab_live_credential_validation_recv_window_ms", 5000),
        ...
        audit = repository.add_audit_event(
            ...
            idempotency_key_hash=hash_idempotency_key(f"live-credential:validate:{request.idempotency_key}"),
    ...
    evidence = build_fake_permission_evidence(
        can_withdraw=request.actor.startswith("local"),
        margin_or_futures_enabled=False,
    )
    ...
    credential.last_validation_reason_code = "live_credential_unsafe_permissions" if new_status == "unsafe_permissions" else "live_credential_fake_validated"
    audit = repository.add_audit_event(
        credential_ref_id=credential.id,
        action="live_credential_validation_completed",
        actor=request.actor,
        environment=LIVE_CREDENTIAL_ENVIRONMENT,
    ...
    return LiveCredentialMutationResult(status="validated", reason_code=credential.last_validation_reason_code, credential_ref_id=str(credential.id), credential_status=credential.status, audit_event_ids=[str(audit.id)], should_commit=True)


def rotate_live_credential(
    repository: Any,
    provider: CredentialVaultProvider,
    credential_ref_id: UUID,
    *,
    ...
    if not request.confirm_rotate:
        return LiveCredentialMutationResult(status="blocked", reason_code="live_credential_rotate_confirmation_required", semantic_status_code=400)
    try:
    ...
    reason_code = "live_credential_fake_rotated"
    ...
    if write.vault_provider == LOCAL_DEV_VAULT_PROVIDER:
        ...
        reason_code = "live_credential_secret_rotated"
    ...
    audit = repository.add_audit_event(
        credential_ref_id=credential.id,
        action="live_credential_rotated",
        actor=request.actor,
        environment=LIVE_CREDENTIAL_ENVIRONMENT,
        reason_code=reason_code,
        idempotency_key_hash=hash_idempotency_key(f"live-credential:rotate:{request.idempotency_key}"),
        metadata=audit_metadata,
    )
    return LiveCredentialMutationResult(status="rotated", reason_code=reason_code, safety_status=_safety_status_for_provider(write.vault_provider), credential_ref_id=str(credential.id), vault_provider=credential.vault_provider, vault_secret_ref=credential.vault_secret_ref, credential_status=credential.status, audit_event_ids=[str(audit.id)], should_commit=True)


def revoke_live_credential(repository: Any, provider: CredentialVaultProvider, credential_ref_id: UUID, *, request: LiveCredentialRevokeRequestData) -> LiveCredentialMutationResult:
    credential = repository.get_credential_ref(credential_ref_id)
    if credential is None:
        return LiveCredentialMutationResult(status="not_found", reason_code="live_credential_not_found", semantic_status_code=404)
    if not request.confirm_revoke:
        return LiveCredentialMutationResult(status="blocked", reason_code="live_credential_revoke_confirmation_required", semantic_status_code=400)
    ...
    audit = repository.add_audit_event(
        credential_ref_id=credential.id,
        action="live_credential_revoked",
        actor=request.actor,
        environment=LIVE_CREDENTIAL_ENVIRONMENT,
        reason_code="live_credential_fake_revoked" if credential.vault_provider == FAKE_VAULT_PROVIDER else "live_credential_secret_revoked",
        idempotency_key_hash=hash_idempotency_key(f"live-credential:revoke:{request.idempotency_key}"),
        metadata=metadata,
    )
    return LiveCredentialMutationResult(status="revoked", reason_code=audit.reason_code, safety_status=_safety_status_for_provider(credential.vault_provider), credential_ref_id=str(credential.id), credential_status=credential.status, audit_event_ids=[str(audit.id)], should_commit=True)


def _read_blocked(repository: Any, request: LiveCredentialReadRequestData, credential_ref_id: UUID | None, reason_code: str = "live_credential_vault_read_blocked") -> LiveCredentialReadResult:
    audit = repository.add_audit_event(
        credential_ref_id=credential_ref_id,
        action="live_credential_vault_read_blocked",
        actor=request.actor,
        environment=LIVE_CREDENTIAL_ENVIRONMENT,
        reason_code=reason_code,
    ...
    return LiveCredentialReadResult(status="blocked", reason_code=reason_code, audit_event_ids=[str(audit.id)], semantic_status_code=403)


def read_live_credential_secret_for_internal_purpose(
    repository: Any,
    provider: CredentialVaultProvider,
    *,
    request: LiveCredentialReadRequestData,
) -> LiveCredentialReadResult:
    requested = repository.add_audit_event(
        credential_ref_id=request.credential_ref_id,
        action="live_credential_vault_read_requested",
        actor=request.actor,
        environment=LIVE_CREDENTIAL_ENVIRONMENT,
        reason_code="live_credential_vault_read_requested",
        request_id=request.request_id,
        metadata={"purpose": request.purpose},
    )
    credential = repository.get_credential_ref(request.credential_ref_id)
    if credential is None:
        blocked = _read_blocked(repository, request, request.credential_ref_id, "live_credential_not_found")
        return LiveCredentialReadResult(status=blocked.status, reason_code=blocked.reason_code, audit_event_ids=[str(requested.id), *blocked.audit_event_ids], semantic_status_code=404)
    if request.purpose not in ALLOWED_LIVE_CREDENTIAL_READ_PURPOSES:
        blocked = _read_blocked(repository, request, credential.id)
        return LiveCredentialReadResult(status=blocked.status, reason_code=blocked.reason_code, audit_event_ids=[str(requested.id), *blocked.audit_event_ids], semantic_status_code=403)
    if credential.environment != LIVE_CREDENTIAL_ENVIRONMENT or credential.vault_provider != LOCAL_DEV_VAULT_PROVIDER:
        blocked = _read_blocked(repository, request, credential.id)
        return LiveCredentialReadResult(status=blocked.status, reason_code=blocked.reason_code, audit_event_ids=[str(requested.id), *blocked.audit_event_ids], semantic_status_code=403)
    if not getattr(credential, "is_active", False) or getattr(credential, "is_deleted", False) or _status_blocks_secret_read(status=credential.status, purpose=request.purpose):
        blocked = _read_blocked(repository, request, credential.id)
        return LiveCredentialReadResult(status=blocked.status, reason_code=blocked.reason_code, audit_event_ids=[str(requested.id), *blocked.audit_event_ids], semantic_status_code=403)
    ...
    try:
        ...
        failed = repository.add_audit_event(
            credential_ref_id=credential.id,
            action="live_credential_vault_read_failed",
            actor=request.actor,
            environment=LIVE_CREDENTIAL_ENVIRONMENT,
            reason_code="live_credential_vault_read_failed",
            request_id=request.request_id,
            metadata={"purpose": request.purpose},
        )
        return LiveCredentialReadResult(status="failed", reason_code="live_credential_vault_read_failed", audit_event_ids=[str(requested.id), str(failed.id)], semantic_status_code=500)
    allowed = repository.add_audit_event(
        credential_ref_id=credential.id,
        action="live_credential_vault_read_allowed",
        actor=request.actor,
        environment=LIVE_CREDENTIAL_ENVIRONMENT,
        reason_code="live_credential_vault_read_allowed",
        request_id=request.request_id,
        metadata={"purpose": request.purpose},
    )
    return LiveCredentialReadResult(status="allowed", reason_code="live_credential_vault_read_allowed", payload=payload, audit_event_ids=[str(requested.id), str(allowed.id)])
