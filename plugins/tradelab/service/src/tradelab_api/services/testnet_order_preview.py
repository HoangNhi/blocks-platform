...
@dataclass(frozen=True)
class TestnetOrderPreviewResult:
    status: str
    allowed: bool
    reason_code: str
...
def preview_testnet_order(
    ...
    if credential is None or getattr(credential, "is_deleted", False):
        return _blocked("testnet_order_preview_credential_not_found", 404)
    ...
    if existing_intent is not None:
        try:
            ensure_same_intent_submit_allowed(existing_intent.status)
        except Exception:
            return _blocked("testnet_order_requires_reconciliation_before_submit", 409)
        ...
        if replay is not None:
            if replay.preview_key != _preview_key(request):
                return _blocked("testnet_order_preview_idempotency_conflict", 409)
            return _allowed_from_rows(existing_intent, replay, credential_snapshot)

    client_order_id = build_client_order_id(intent_key)
    source_snapshot = _source_snapshot(request)
    risk_snapshot = {"passed": True, "reasonCode": "testnet_order_risk_gate_passed", "failedGates": []}
    ...
    intent = existing_intent or order_repository.create_intent(
        ...
        status_reason_code="testnet_order_preview_allowed",
...
def _validate_request_shape(request: TestnetOrderPreviewRequestData) -> TestnetOrderPreviewResult | None:
    if not request.confirm_preview_only:
        return _blocked("testnet_order_preview_confirmation_required", 400)
    if not request.idempotency_key.strip():
        return _blocked("testnet_order_preview_idempotency_required", 400)
    if len(request.idempotency_key) > 120 or is_secret_like_key(request.idempotency_key):
        return _blocked("testnet_order_preview_idempotency_invalid", 400)
    if request.environment != "binance_testnet" or request.exchange != "binance" or request.market_type != "spot":
        return _blocked("testnet_order_preview_live_route_blocked", 400)
    if request.order_type.lower() != "market" or request.side.lower() not in {"buy", "sell"}:
        return _blocked("testnet_order_preview_risk_gate_failed", 400)
    if not request.symbol.strip():
        return _blocked("testnet_order_preview_symbol_not_allowed", 400)
    has_quantity = request.quantity is not None
    has_quote = request.quote_quantity is not None
    if has_quantity == has_quote:
        return _blocked("testnet_order_preview_quantity_invalid", 400)
...
def _credential_block_reason(credential: Any) -> str | None:
    ...
    if credential.environment != "binance_testnet" or credential.exchange != "binance_spot":
        return "testnet_order_preview_live_route_blocked"
    if credential.status not in READY_CREDENTIAL_STATUSES:
        return "testnet_order_preview_credential_not_ready"
    if not evidence.get("canTrade"):
        return "testnet_order_preview_credential_not_ready"
    if evidence.get("marginOrFuturesEnabled"):
        return "testnet_order_preview_unsafe_permissions"
    return None
...
def _source_snapshot(request: TestnetOrderPreviewRequestData) -> dict[str, Any]:
    return {
...
def _preview_key(request: TestnetOrderPreviewRequestData) -> str:
    payload = json.dumps({
        "strategyId": str(request.strategy_id),
        "strategyVersionId": str(request.strategy_version_id),
...
def _blocked(reason_code: str, semantic_status_code: int, *, credential_snapshot: dict[str, Any] | None = None) -> TestnetOrderPreviewResult:
    return TestnetOrderPreviewResult(status="blocked", allowed=False, reason_code=reason_code, semantic_status_code=semantic_status_code, credential_snapshot=credential_snapshot or {}, risk_snapshot={"passed": False, "reasonCode": reason_code, "failedGates": [reason_code]})

def _allowed_from_rows(intent: Any, preview: Any, credential_snapshot: dict[str, Any]) -> TestnetOrderPreviewResult:
    return _result_from_rows(intent, preview, credential_snapshot, [], should_commit=False, reason_code="testnet_order_preview_idempotency_replayed")

def _result_from_rows(intent: Any, preview: Any, credential_snapshot: dict[str, Any], audit_event_ids: list[str], *, should_commit: bool, reason_code: str | None = None) -> TestnetOrderPreviewResult:
    return TestnetOrderPreviewResult(
        status="previewed",
        allowed=True,
        reason_code=reason_code or preview.reason_code or "testnet_order_preview_allowed",
