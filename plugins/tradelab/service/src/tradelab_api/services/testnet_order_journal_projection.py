...
from tradelab_api.services.execution_journal import (
    ASSISTED_TESTNET_EXECUTION_JOURNAL_SAFETY_STATUS,
    JournalFillInput,
    build_assisted_testnet_planned_snapshot,
    derive_comparison_summary,
)

TERMINAL_TESTNET_ORDER_STATUSES = {"filled", "cancelled", "rejected", "reconciled"}
...
class TestnetOrderJournalProjectionRequestData:
    __test__ = False

    order_id: UUID
    confirm_testnet_journal_projection: bool
    source: str = "strategy_lab"
    actor: str = "local-user"


@dataclass(frozen=True)
class TestnetOrderJournalProjectionResult:
    __test__ = False

    status: str
    reason_code: str
    safety_status: str = ASSISTED_TESTNET_EXECUTION_JOURNAL_SAFETY_STATUS
...
def project_testnet_order_to_journal(
    *,
    ...
    if not request.confirm_testnet_journal_projection:
        return _blocked("testnet_order_journal_projection_confirm_required")
    if request.source != "strategy_lab":
        return _blocked("testnet_order_journal_projection_source_not_supported")

    intent = order_repository.get_intent(request.order_id)
    if intent is None:
        return _blocked("testnet_order_not_found", semantic_status_code=404)
    if getattr(intent, "journal_entry_id", None):
        return _blocked("testnet_order_journal_projection_duplicate", intent=intent, semantic_status_code=409)
    if getattr(intent, "status", None) not in TERMINAL_TESTNET_ORDER_STATUSES:
        return _blocked("testnet_order_journal_projection_non_terminal", intent=intent)

    source_run_id = getattr(intent, "source_run_id", None)
    if source_run_id is None:
        return _blocked("testnet_order_journal_projection_source_run_required", intent=intent)
    run = run_repository.get_run(source_run_id)
    if run is None:
        return _blocked("testnet_order_journal_projection_source_run_not_found", intent=intent, semantic_status_code=404)
    if getattr(run, "status", None) != "completed":
        return _blocked("testnet_order_journal_projection_source_run_not_completed", intent=intent)
    ...
    planned_snapshot = build_assisted_testnet_planned_snapshot(run, intent=intent, evidence=evidence)
    ...
    entry = journal_repository.create_entry(
        ...
        discipline_status="not_recorded",
        safety_status=ASSISTED_TESTNET_EXECUTION_JOURNAL_SAFETY_STATUS,
        notes="Projected from assisted Binance Spot Testnet order evidence.",
        fills=fill_rows,
    ...
    event = order_repository.add_event(
        ...
        event_type="testnet_order_journal_projection_planned",
...
def _blocked(reason_code: str, *, intent: object | None = None, semantic_status_code: int = 400) -> TestnetOrderJournalProjectionResult:
    return TestnetOrderJournalProjectionResult(
        status="blocked",
