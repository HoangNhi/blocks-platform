...
def get_testnet_order_detail(repository: TestnetOrderStateRepository, order_id: UUID) -> dict | None:
...
def list_testnet_orders(
    repository: TestnetOrderStateRepository,
    *,
    ...
    items = []
    for intent in repository.list_intents(strategy_id=strategy_id, strategy_version_id=strategy_version_id, source_run_id=source_run_id, credential_ref_id=credential_ref_id, status=status, symbol=symbol, limit=limit):
    ...
    return {"safetyStatus": "assisted_testnet_order_list_read_only", "items": items}
