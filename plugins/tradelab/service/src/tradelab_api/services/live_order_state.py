...
class LiveOrderStateError(Exception):
    __test__ = False

    def __init__(self, *, reason_code: str, from_status: str, to_status: str | None = None) -> None:
...
def transition_order_status(from_status: str, to_status: str) -> str:
    ...
    if allowed is None or to_status not in allowed:
        raise LiveOrderStateError(
            reason_code="live_order_invalid_state_transition",
...
def ensure_same_intent_submit_allowed(status: str) -> None:
    if status in SUBMIT_BLOCKED_STATUSES:
        raise LiveOrderStateError(
            reason_code="live_order_requires_reconciliation_before_submit",
            from_status=status,
        )
