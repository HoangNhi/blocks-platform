...
from tradelab_api.services.testnet_order_preview import TestnetOrderPreviewRequestData as PreviewRequestData, preview_testnet_order  # noqa: E402
...
def _request(session: Session, **overrides) -> PreviewRequestData:
    ...
    values = dict(confirm_preview_only=True, idempotency_key="preview-key-1", client_action_id="action-1", source="strategy_lab", actor="admin", strategy_id=strategy.id, strategy_version_id=version.id, source_run_id=None, source_signal_package_id=None, credential_ref_id=credential_ref_id, environment="binance_testnet", exchange="binance", market_type="spot", symbol="BTCUSDT", side="buy", order_type="market", quantity=None, quote_quantity=Decimal("25"))
...
def test_allowed_preview_persists_intent_preview_and_event(db_session: Session) -> None:
    ...
    assert result.client_order_id.startswith("tltn-")
...
def test_preview_allows_validated_spot_testnet_credential_with_withdraw_flag(db_session: Session) -> None:
    strategy, version = _strategy(db_session)
    credential = _credential(db_session, status="validated_testnet_read_only", can_withdraw=True)
...
def test_preview_blocks_margin_enabled_credential(db_session: Session) -> None:
    strategy, version = _strategy(db_session)
    credential = _credential(db_session, status="validated_testnet_read_only", can_withdraw=True)
