...
def _request(confirm: bool = True) -> LiveOrderJournalProjectionRequestData:
    return LiveOrderJournalProjectionRequestData(
        order_id=uuid4(),
        confirm_live_journal_projection=confirm,
        source="strategy_lab",
        actor="local-user",
    )
...
def test_projection_blocks_without_confirmation() -> None:
    ...
    assert result.reason_code == "live_order_journal_projection_confirm_required"
...
def test_projection_blocks_non_terminal_status(status: str) -> None:
    ...
    assert result.status == "blocked"
    assert result.reason_code == "live_order_journal_projection_non_terminal"
...
def test_projection_creates_journal_entry_and_marks_order_projected() -> None:
    ...
    assert result.reason_code == "live_order_journal_projection_created"
    ...
    assert journal_repository.entries[0]["planned_snapshot"]["source"] == "assisted_live_order"
    assert journal_repository.entries[0]["planned_snapshot"]["liveOrderIntentId"] == str(order_repository.intent.id)
    assert order_repository.events[0]["event_type"] == "live_order_journal_projection_planned"
