...
from tradelab_api.services.live_order_cancel import LiveOrderCancelRequestData, cancel_live_order  # noqa: E402
...
def _submitted_context(session: Session, *, status: str = "submitted", local_dev_secret: bool = False, reconciliation_required: bool = False):
...
def _request(order_id, **overrides) -> LiveOrderCancelRequestData:
    ...
    values.update(overrides)
    return LiveOrderCancelRequestData(**values)
...
def test_cancel_requires_confirmation_and_idempotency(db_session: Session) -> None:
    ...
    missing_confirm = cancel_live_order(repository, credential_repository, _request(intent.id, confirm_live_cancel=False))
    missing_key = cancel_live_order(repository, credential_repository, _request(intent.id, idempotency_key=""))

    assert missing_confirm.status == "blocked"
    assert missing_confirm.reason_code == "live_order_cancel_confirmation_required"
    assert missing_key.reason_code == "live_order_cancel_idempotency_required"
...
def test_cancel_blocks_non_cancellable_state_before_vault_or_network(db_session: Session) -> None:
    ...
    result = cancel_live_order(repository, credential_repository, _request(intent.id), vault_provider=vault)  # type: ignore[arg-type]

    assert result.status == "blocked"
    assert result.reason_code == "live_order_cancel_state_not_allowed"


def test_cancel_fake_order(db_session: Session) -> None:
    repository, credential_repository, intent, *_ = _submitted_context(db_session, status="submitted")

    result = cancel_live_order(repository, credential_repository, _request(intent.id, connector_mode="fake"))

    assert result.status == "cancelled"
    assert result.reason_code == "live_order_cancel_fake_accepted"
...
def test_cancel_real_mode_network_disabled_blocks_before_vault(db_session: Session) -> None:
    ...
    result = cancel_live_order(repository, credential_repository, _request(intent.id, connector_mode="real", real_network_enabled=False))

    assert result.status == "blocked"
    assert result.reason_code == "live_order_cancel_real_network_not_enabled"
...
def test_cancel_real_accepted_persists_cancelled_and_sanitized_event(db_session: Session) -> None:
    ...
    assert result.status == "cancelled"
    assert result.safety_status == "assisted_live_cancel_live_only"
